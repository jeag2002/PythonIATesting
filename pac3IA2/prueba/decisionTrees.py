...
def DTs(train, test):
   ...
   clasTrain = list(map(lambda x: x.pop(0), atrsTrain))
   atrsTest = deepcopy(test)
   clasTest = list(map(lambda x: x.pop(0), atrsTest))
   tree = iteration(clasTrain, list(zip(*atrsTrain)))
   preds = [testEx(ex, tree) for ex in atrsTest]
   res = float(len(list(filter(lambda x: x[0] == x[1], zip(*[preds, clasTest]))))) / float(len(clasTest))
   return [tree, res]
